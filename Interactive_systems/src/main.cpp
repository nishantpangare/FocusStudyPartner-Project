...
// ───────────── BUZZER UTIL ─────────────
// Single short beep — only ever called once, right when
// absence is first detected and the countdown begins
void buzzerBeepOnce() {
...
void handlePresenceMonitoring() {
  ...
  if (timeSinceLastSeen > CHECK_INTERVAL_MS) {
    ...
    if (timeSinceLastSeen >= ABSENCE_TIMEOUT_MS) {
      // ── 30 seconds passed with no detection → PAUSE session ──
      if (!wasAbsent) {
        wasAbsent      = true;
        isCountingDown = false;
      ...
      // ── Inside the 0-30s grace window — beep once + show countdown ──
      if (!isCountingDown) {
        isCountingDown = true;
        buzzerBeepOnce();   // single beep, only when countdown first starts
